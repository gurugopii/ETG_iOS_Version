...
#import "ETGProjectScheduleProgress.h"
#import "ETGWebServiceCommonImports.h"
//Models
#import "ETGProjectSummary.h"
#import "ETGScheduleProgress_Project.h"

#define ETG_PROJECT_SCHEDULE_PROGRESS_PATH @"/Service1.svc/"

@interface ETGProjectScheduleProgress ()
...
@implementation ETGProjectScheduleProgress
...
- (id)init {
    ...
    if (self) {
        ...
        if (!self.isMapped) {
            [self setupScheduleProgressMappings];
            self.isMapped = YES;
        }
    }
    return self;
...
- (void)setupScheduleProgressMappings {
    
    // Attribute Mappings for Schedule Progress
    RKEntityMapping *scheduleMapping = [[RKEntityMapping alloc] initWithEntity:[ETGScheduleProgress_Project entityInManagedObjectContext:[NSManagedObjectContext defaultContext]]];
    ...
    RKResponseDescriptor *scheduleDescriptor = [RKResponseDescriptor responseDescriptorWithMapping:scheduleMapping method:RKRequestMethodGET pathPattern:ETG_PROJECT_SCHEDULE_PROGRESS_PATH keyPath:nil statusCodes:RKStatusCodeIndexSetForClass(RKStatusCodeClassSuccessful)];
    [_managedObject addResponseDescriptor:scheduleDescriptor];
    
    // Attribute Mappings from core data to webview(html)
    RKEntityMapping *scheduleMapping_ = [[RKEntityMapping alloc] initWithEntity:[ETGScheduleProgress_Project entityInManagedObjectContext:[NSManagedObjectContext defaultContext]]];
...
-(void) sendRequestWithReportingMonth:(NSString *)reportingMonth
    ...
    RKManagedObjectRequestOperation *scheduleOperation = [_managedObject appropriateObjectRequestOperationWithObject:nil method:RKRequestMethodGET path:ETG_PROJECT_SCHEDULE_PROGRESS_PATH parameters:params];
    ...
    void (^successBlock)(RKObjectRequestOperation*, RKMappingResult*) = ^(RKObjectRequestOperation *scheduleOperation, RKMappingResult *mappingResult) {
        ...
        // Add all ETGScheduleProgress_Project to ETGProjectSummary
        [project addEtgScheduleProgress_Projects:[mappingResult set]];
        
        // Add ETGProjectSummary to ETGScheduleProgress_Project
        for (ETGScheduleProgress_Project *schedule in [mappingResult set]) {

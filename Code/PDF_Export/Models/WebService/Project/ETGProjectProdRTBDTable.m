...
@implementation ETGProjectProdRTBDTable
...
- (id)init {
    ...
    if (self) {
        ...
        if (!self.isMapped) {
            [self setupProductionRtbdTableMappings];
            self.isMapped = YES;
        }
    }
    return self;
...
- (void)setupProductionRtbdTableMappings {
    
    // Attribute Mappings for Production and RTBD Table
    RKEntityMapping *productionRtbdMapping = [[RKEntityMapping alloc] initWithEntity:[ETGProductionRtbdTable_Project entityInManagedObjectContext:[NSManagedObjectContext defaultContext]]];
    ...
    RKEntityMapping *productionRtbdMapping_ = [[RKEntityMapping alloc] initWithEntity:[ETGProductionRtbdTable_Project entityInManagedObjectContext:[NSManagedObjectContext defaultContext]]];
...
-(void) sendRequestWithReportingMonth:(NSString *)reportingMonth
    ...
    void (^successBlock)(RKObjectRequestOperation*, RKMappingResult*) = ^(RKObjectRequestOperation *productionRtbdOperation, RKMappingResult *mappingResult) {
        ...
        // Add all ETGProductionRtbdTable_Project to ETGProjectSummary
        [project addEtgProductionRtbdTable_Projects:[mappingResult set]];
        
        // Add ETGProjectSummary to ETGProductionRtbdTable_Project
        for (ETGProductionRtbdTable_Project *prodRtbd in [mappingResult set]) {

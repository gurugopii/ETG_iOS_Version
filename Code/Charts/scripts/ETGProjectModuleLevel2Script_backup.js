...
jQuery(function ($) {
  ...
  if (isFromPortfolio) {
    window.swiper.slide(pageIndex, function () {
...
function populateAllChartsAtLevel2(contentJSONStr, isFromPortfolioCharts, chartType) {
 ...
 //getting all data for all charts
  ...
  var level = 2;
  var pageCount = 0;
  var pagePosition = ['#first', '#second', '#third', '#fourth', '#fifth', '#sixth', '#seventh', '#eighth'];
...
//      else { console.log("HSE detailed table is disabled"); }
  ...
  //remove extra pages
  if (pageCount < 8) {
    for (var pageCountIndex = pageCount; pageCountIndex <= 8; pageCountIndex++) {
  ...
  if(validateField(isFromPortfolioCharts)){
